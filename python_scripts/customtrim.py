...
def read_taxa_list(list_file):
    ...
    logging.info("read", len(taxalist), "records")
    return(taxalist)
...
def main():
    ...
    if len(sys.argv) >= 3:
        ...
        if len(files) == 0:
            logging.error("no fasta files in the directory")
            sys.exit()
        else:
            trimopt = sys.argv[2]
            if trimopt == "-a" or trimopt == "-1":
                exclusion_file = read_taxa_list(sys.argv[3])
            elif trimopt[:3] == "-d%":
                exclusion_file = read_taxa_list(sys.argv[3])
                ff = sys.argv[4]
                if ff == "dna":
                ...
                if ff == "dna":
                    ...
                    alph = Gapped(IUPAC.protein, '-')
            logging.info("the option", trimopt, "set up")
            if trimopt[:2] == "-%":
                logging.info("the cutoff is set to", trimopt[2:])
            elif trimopt[:3] == "-d%":
                logging.info("the cutoff is set to", trimopt[3:])
            
                
    else:
        print("FORMAT: python customtrim.py [folder with fasta] " +
              "[trimming option: -a, -1, -%, -refine, -d, -d%, -tiger] " +
              "([trimlist])")
