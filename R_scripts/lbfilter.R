...
#go over branches, splitting if exceeding mean branch length times threshold
#until no more splits
#if return max, returns the largest tree (default lbfilter.R script behavior) 
...
#adjust threshold by length
#coefficients were determined empirically
adjusted_thresh <- threshval / 200 * len_adjustment
#run tree splitter
trsout <- splittrees(list(trs), adjusted_thresh, T)

#trim the sequence alignment
