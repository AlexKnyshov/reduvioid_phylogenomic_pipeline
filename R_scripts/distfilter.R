...
argsLen <- length(args);
if (argsLen == 0){
  cat("Syntax: Rscript distfilter.R [path to concat dna] [path to dna folder] [path to concat protein] [path to protein folder] ([upperN] ([lowerN] ([WAG]))) \n")
...
#separate function to compute AA dist matrix
#instead of dist.aa divided by gene length
#use dist.ml with model WAG this is more accurate
...
for (i in 1:length(commonfiles)){
	...
	if (length(dnalocus[,1]) > 1 & length(proteinlocus[,1]) > 1){
		...
		#get difference and find outliers
		...
		if (length(outliers) / length(nameorder) < 0.5){
			# less than 50% outliers	
			if (length(outliers)>0){
				#some outliers - correct alignment
			...
			# over 50% outliers - do not output (remove)
			print(paste0("removed file: ", commonfiles[i], length(outliers),
				", NTU: ", NdnaU, ", NTL: ", NdnaL, ", AAU: ", NproteinU, ", AAL: ", NproteinL))
		}
	} else {
		# the file was empty and was removed
